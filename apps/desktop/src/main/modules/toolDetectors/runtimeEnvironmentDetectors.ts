...
/**
 * nexumChat CLI detector
 * Tries lobehub, lobe, lh in order; validates via --help output containing "nexumChat" or "LobeHub"
 */
export const lobehubDetector: IToolDetector = {
  description: 'nexumChat CLI - manage and connect to nexumChat services',
  async detect(): Promise<ToolStatus> {
    ...
    for (const cmd of commands) {
      try {
        ...
        // Validate it's actually nexumChat CLI by checking help output
        const { stdout: helpOut } = await execPromise(`${cmd} --help`, { timeout: 3000 });
        if (!helpOut.includes('LobeHub') && !helpOut.includes('nexumChat')) continue;
