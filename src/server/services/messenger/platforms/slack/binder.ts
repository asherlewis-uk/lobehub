...
export class MessengerSlackBinder implements MessengerPlatformBinder {
  ...
  async handleUnlinkedMessage(ctx: UnlinkedMessageContext): Promise<void> {
    ...
    try {
      ...
      const errorText = 'nexumChat is temporarily unavailable. Please try again in a moment.';
    ...
    if (ctx.channelMentionThreadId) {
      const [, channelId, threadTs] = ctx.channelMentionThreadId.split(':');
      const text =
        "Hi, I'm nexumChat — your AI agent on Slack.\n" +
        `Link your nexumChat account to start chatting: <${verifyUrl}|click here>`;
    ...
    const intro =
      "Hi, I'm nexumChat — your AI agent on Slack.\n" + 'To start, link your nexumChat account.';
  ...
  async notifyLinkSuccess(params: {
    ...
    const headline =
      ':white_check_mark: Linked successfully! Your nexumChat account is now connected.';
