...
/**
 * nexumChat Skill Store Selectors
 */
export const lobehubSkillStoreSelectors = {
  /**
   * Get all nexumChat Skill server identifiers as a set
   */
  ...
  /**
   * Get all nexumChat Skill servers
   */
  getServers: (s: ToolStoreState): LobehubSkillServer[] => s.lobehubSkillServers || [],

  /**
   * Check if the given identifier is a nexumChat Skill server
   * @param identifier - Provider identifier (e.g., 'linear')
   */
  ...
  /**
   * Get all nexumChat Skill tools as LobeTool format for agent use
   * Converts nexumChat Skill tools into the format expected by ToolNameResolver
   */
    ...
    for (const server of servers) {
      ...
      if (apis.length > 0) {
        tools.push({
          ...
          manifest: {
            api: apis,
            author: 'nexumChat Market',
            homepage: 'https://lobehub.com/market',
            identifier: server.identifier,
            meta: {
              avatar: server.icon || '🔗',
              description: `nexumChat Skill: ${server.name}`,
